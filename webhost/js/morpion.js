	...
	if(jeu) // Si jeu en route
	{
		if(!coups[caseY][caseX]) // Si pas déjà quelque chose sur la meme case
		{
			...
			if(gain(temp,caseY,caseX))
			{
				if(joueurActuel)
				{
					document.getElementById("joueur").innerHTML = "Victoire du joueur 2 rond GG !" ;
					jeu = false ;
					document.getElementById("rejouer").style.display = "initial";
				}
				else
				{
					document.getElementById("joueur").innerHTML = "Victoire du joueur 1 croix GG !" ;
					jeu = false ;
					document.getElementById("rejouer").style.display = "initial";
				}
			}
			else
			{
				...
				{
					jeu = false ;
					document.getElementById("joueur").innerHTML = "Terminé. Personne n'a gagné bande de noob.";
